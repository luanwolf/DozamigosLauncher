...
export const ZoneNames = derived(
  ...
  ([$language, $t]) =>
    ({
      ...
      'mini-boss': missions.fightCategory4Storm.names[$language],
      'yarrr-island': missions['yarrr!'].names[$language],
      'walk-plank': missions.walkThePlank.names[$language],
      'adventure-revenge': missions.exploreTheMist.names[$language],
      horde: missions.challengeTheHorde.names[$language],
      onboarding: missions.onboardingFort.names[$language],
      hestia: missions.deliverTheGoods.names[$language],
      quest: $t('quest')
    }) as Record<string, string>
);

/** Fallback icon when a zone category has no dedicated asset under /static/world. */
export const DefaultMissionZoneIcon = '/world/quest.png';

/** Zone categories that should always use {@link DefaultMissionZoneIcon}. */
export const ZoneCategoriesWithoutIcon = new Set<keyof typeof ZoneCategories>([
  'quest',
  'yarrr-island',
  'walk-plank',
  'adventure-revenge',

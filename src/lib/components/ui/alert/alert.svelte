...
<div class={['rounded-none border p-3', backgroundcolors[color], !title && Icon ? 'flex items-center gap-2' : '']}>
  {#if title}
    <div class="mb-1 flex items-center gap-2">
      ...
      <span class="font-display text-sm font-bold tracking-wide">{title}</span>
    </div>
  {:else if Icon}
    <Icon class="size-5 shrink-0 {iconColors[color]}" />
  {/if}

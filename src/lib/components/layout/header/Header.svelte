<script lang="ts">
  ...
  import NotificationsPanel from '$components/layout/header/NotificationsPanel.svelte';
...
<header
  class="app-shell-header sticky top-0 z-[60] flex min-w-0 shrink-0 items-center gap-1.5 overflow-hidden border-b border-border/70 bg-sidebar/95 px-2 backdrop-blur-md select-none sm:gap-2 sm:px-4"
...
>
  ...
  {#if import.meta.env.DEV}
    <span
      class="hidden border border-primary/50 bg-primary px-1.5 py-0.5 font-display text-sm leading-none text-primary-foreground min-[700px]:inline"
    >
      DEV
    </span>
  {/if}
  ...
  <div class="ml-auto flex shrink-0 items-center gap-x-1 sm:gap-x-2">
    ...
    <div class="hidden items-center min-[520px]:flex">
      <button
        class="p-2 transition-colors duration-200 hover:bg-accent"
      ...
      <button
        class="p-2 transition-colors duration-200 hover:bg-destructive hover:text-white"
        onclick={close}
        type="button"
      >
        <XIcon class="size-4" />

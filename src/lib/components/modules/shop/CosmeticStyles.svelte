  ...
  <div class="space-y-2">
    ...
    <div class="flex flex-wrap gap-2">
      {#each styles as style (style.image)}
        <button
          class="w-24 space-y-1 rounded-md border p-1 text-left transition-colors hover:border-primary
        ...
        >
          <img
            ...
            src={style.image}
          />
          <span class="block text-center text-xs leading-tight text-muted-foreground" title={style.name}>
            {style.name}
          </span>
        </button>
      {/each}
